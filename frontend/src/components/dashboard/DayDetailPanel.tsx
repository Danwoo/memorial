...
import { fetchDigestByDate, fetchJournalsByDate } from '../../api'
...
export default function DayDetailPanel({
  ...
  const memoryCount = digest?.summary.memory_count ?? 0
  const journalCount = journals.length
  const memories = digest?.memories ?? []
  const showInsights = isToday(date) && dailyInsights.length > 0
  const isEmpty = !loading && journalCount === 0 && memoryCount === 0
            ...
            <div className="day-panel__summary">
              {journalCount > 0 && (
                <span className="day-panel__summary-item">
                  <Pencil size={14} /> 다이어리 {journalCount}개
                </span>
              )}
              {memoryCount > 0 && (
                <span className="day-panel__summary-item">
                  <BookOpen size={14} /> 스크랩 {memoryCount}개
                </span>
              )}
            </div>

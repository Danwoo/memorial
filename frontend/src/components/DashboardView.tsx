...
import { fetchStreak, fetchStats, fetchActivity, fetchBriefing, fetchDailyInsights, fetchJournalDates } from '../api'
...
import DayDetailPanel from './dashboard/DayDetailPanel'
...
function getBannerMessage(briefing: BriefingData | null): string | null {
  ...
  const { today_memories, connection_hint } = briefing
  if (connection_hint) return connection_hint
  if (today_memories.count > 0) {
    const topics = today_memories.topics.slice(0, 2).join(', ')
    return topics
      ? `오늘 ${topics} 관련 기억 ${today_memories.count}개가 저장됐어요`
      : `오늘 기억 ${today_memories.count}개가 저장됐어요`
  }
  return null
}
...
export default function DashboardView() {
  ...
  const { data: dashData, isLoading: loading } = useViewCache<DashboardCache>({
    ...
    fetcher: async () => {
      try {
        const [briefingData, streakData, statsData, activityData, insightsData, journalDatesData] = await Promise.all([
          ...
          fetchJournalDates(90).catch(() => ({ dates: [] })),
        ])
        return {
  ...
  const handleNavigateJournal = useCallback((date: string) => {
    navigate('/journal', { state: { date } })
  }, [navigate])

  const handleNavigateMemory = useCallback((memoryId: string) => {
    navigate('/memories', { state: { openMemoryId: memoryId } })
  }, [navigate])
  ...
  return (
    <div className="dashboard-view">
      ...
      {/* 캘린더 + DayDetailPanel */}
      <div className={`dashboard-calendar-container${selectedDate ? ' dashboard-calendar-container--panel-open' : ''}`}>
        <div className="dashboard-calendar-main">
          <MonthlyCalendar
            year={calYear}
            month={calMonth}

...
import type { MindmapInsights, ClusterInfo, SearchResult } from '../types'
import { searchScraps, createMindmapRelation } from '../api'
import { useToast } from '../contexts/ToastContext'
import './MindmapInsightPanel.css'
...
export default function MindmapInsightPanel({
  ...
  const handleConnectClick = async (nodeName: string) => {
    ...
    try {
      const res = await searchScraps({ q: nodeName, limit: 5 })
  ...
  const handleCreateConnection = async (targetName: string) => {
    ...
    try {
      await createMindmapRelation(connectingNode, targetName)
            ...
            {insights.clusters.map((cluster, i) => (
              <div key={cluster.cluster_id} className="insight-cluster-card-wrapper">
                ...
                <button
                  ...
                  onClick={() => navigate(demoPath('/diary'), { state: { openSocrates: true, topic: cluster.summary || cluster.entities[0] } })}
                  title="이 주제에 대해 대화하기"
                >
      ...
      {insights.isolated_nodes.length > 0 && (
        <div className="insight-section">
          <div className="insight-section-header">
            ...
            <button className="section-link" onClick={() => navigate(demoPath('/scraps'))}>
              <BookOpen size={12} />
              스크랩 뷰에서 보기
            </button>
          </div>
                ...
                {connectingNode === node.name && (
                  <div className="connect-dropdown">
                    ...
                    {!searchLoading && searchResults.length === 0 && (
                      <span className="connect-empty">유사 스크랩 없음</span>
                    )}
                    {!searchLoading && searchResults.map(r => (
                      ...
                      >
                        {r.title || r.content?.substring(0, 40) || '스크랩'}
                      </button>
                    ))}
                  </div>
                )}

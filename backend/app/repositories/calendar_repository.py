...
class CalendarRepository:
    ...
    def _select_all(self, user_id: str):
        return self.db.table("scraps").select("*").eq("user_id", user_id).execute()
    ...
    def _select_source_types(self, user_id: str):
        return self.db.table("scraps").select("source_type").eq("user_id", user_id).execute()

    def _select_tags(self, user_id: str):
        return self.db.table("scraps").select("tags").eq("user_id", user_id).execute()
    ...
    async def get_all_active_dates(self, user_id: UUID) -> set[str]:
        """스크랩 + 일기에서 활동한 모든 날짜(YYYY-MM-DD) 집합 반환."""
    ...
    def _select_memory_dates(self, user_id: str):
        return self.db.table("scraps").select("created_at").eq("user_id", user_id).execute()

    def _select_journal_dates(self, user_id: str):
        return self.db.table("diaries").select("created_at").eq("user_id", user_id).execute()

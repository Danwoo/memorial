...
class DigestService:
    ...
    async def get_today_digest(self, user_id: UUID, target_date: datetime | None = None) -> dict[str, Any]:
        ...
        chats = await self._get_today_chats(user_id, today_start, today_end)
    ...
    async def _get_today_chats(self, user_id: UUID, today_start: datetime, today_end: datetime) -> list[dict]:
        """오늘 생성된 소크라테스 세션 조회."""

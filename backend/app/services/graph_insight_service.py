...
class GraphInsightService:
    ...
    def __init__(self, graph_repo: GraphRepository, stats_repo: StatsRepository):
        self.graph_repo = graph_repo
        self.stats_repo = stats_repo

    async def get_insights(self, user_id: str) -> GraphInsightsResponse:
    ...
    async def _detect_clusters(self, user_id: str) -> list[ClusterInfo]:
        ...
        edges = await self.graph_repo.get_all_edges(user_id)
        if not edges:
            return []
    ...
    async def _compute_trends(self, user_id: str) -> list[TrendItem]:
        ...
        for week_offset in range(3, -1, -1):
            ...
            memories = await self.stats_repo.get_memories_in_range(
                UUID(user_id),
                start,
                end,
            )
    ...
    async def _find_hub_nodes(self, user_id: str) -> list[HubNode]:
        ...
        hubs = await self.graph_repo.get_hub_nodes(user_id, top_n=5)

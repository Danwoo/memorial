"""
CRUD operations for memories table
Based on Data_Schema.md - Section 2.2
"""
from typing import Optional, List
from uuid import UUID, uuid4
from datetime import datetime, timezone
from supabase import Client

from app.schemas.memory import MemoryCreate, MemoryInDB, SourceType


async def create_memory(
    db: Client,
    user_id: UUID,
    title: str,
    content: str,
    source_type: SourceType,
    source_url: Optional[str] = None,
    summary: Optional[str] = None,
) -> MemoryInDB:
    """
    Create a new memory record in Supabase.
    Returns the created memory with generated ID.
    """
    memory_id = uuid4()
    now = datetime.now(timezone.utc).isoformat()
    
    data = {
        "id": str(memory_id),
        "user_id": str(user_id),
        "title": title,
        "content": content,
        "source_type": source_type,
        # Note: source_url removed - column may not exist in some Supabase schemas
        "summary": summary,
        "status": "processing",
        "created_at": now,
        "updated_at": now,
    }
    
    # Only add source_url if provided (optional column)
    # if source_url:
    #     data["source_url"] = source_url
    
    result = db.table("memories").insert(data).execute()
    
    if result.data:
        row = result.data[0]
        return MemoryInDB(
            id=UUID(row["id"]),
            user_id=UUID(row["user_id"]),
            title=row["title"],
            content=row["content"],
            summary=row.get("summary"),
            source_url=row.get("source_url"),
            source_type=row["source_type"],
            status=row.get("status", "pending"),
            tags=row.get("tags"),
            created_at=datetime.fromisoformat(row["created_at"].replace("Z", "+00:00")),
            updated_at=datetime.fromisoformat(row["updated_at"].replace("Z", "+00:00")) if row.get("updated_at") else None
        )
    
    raise Exception("Failed to create memory")


async def get_memory_by_id(
    db: Client,
    memory_id: UUID,
    user_id: UUID
) -> Optional[MemoryInDB]:
    """
    Get a single memory by ID (with user_id for RLS).
    """
    result = db.table("memories").select("*").eq("id", str(memory_id)).eq("user_id", str(user_id)).single().execute()
    
    if result.data:
        row = result.data
        return MemoryInDB(
            id=UUID(row["id"]),
            user_id=UUID(row["user_id"]),
            title=row["title"],
            content=row["content"],
            summary=row.get("summary"),
            source_url=row.get("source_url"),
            source_type=row["source_type"],
            status=row.get("status", "pending"),
            tags=row.get("tags"),
            created_at=datetime.fromisoformat(row["created_at"].replace("Z", "+00:00")),
            updated_at=datetime.fromisoformat(row["updated_at"].replace("Z", "+00:00")) if row.get("updated_at") else None
        )
    
    return None


async def get_memories_by_user(
    db: Client,
    user_id: UUID,
    page: int = 1,
    limit: int = 20,
    search: Optional[str] = None
) -> tuple[List[MemoryInDB], int]:
    """
    Get paginated list of memories for a user.
    Returns (items, total_count)
    """
    offset = (page - 1) * limit
    
    query = db.table("memories").select("*", count="exact").eq("user_id", str(user_id))
    
    if search:
        query = query.or_(f"title.ilike.%{search}%,content.ilike.%{search}%")
    
    query = query.order("created_at", desc=True).range(offset, offset + limit - 1)
    
    result = query.execute()
    
    items = []
    if result.data:
        for row in result.data:
            items.append(MemoryInDB(
                id=UUID(row["id"]),
                user_id=UUID(row["user_id"]),
                title=row["title"],
                content=row["content"],
                summary=row.get("summary"),
                source_url=row.get("source_url"),
                source_type=row["source_type"],
                status=row.get("status", "pending"),
                tags=row.get("tags"),
                created_at=datetime.fromisoformat(row["created_at"].replace("Z", "+00:00")),
                updated_at=datetime.fromisoformat(row["updated_at"].replace("Z", "+00:00")) if row.get("updated_at") else None
            ))
    
    total = result.count if result.count else 0
    
    return items, total


async def update_memory_status(
    db: Client,
    memory_id: UUID,
    status: str,
    summary: Optional[str] = None,
    tags: Optional[List[str]] = None,
    source_url: Optional[str] = None,
    source_type: Optional[str] = None
) -> bool:
    """
    Update memory status and optionally summary/tags.
    Used after Librarian agent processes the memory.
    """
    now = datetime.now(timezone.utc).isoformat()
    
    update_data = {
        "status": status,
        "updated_at": now
    }
    
    if summary is not None:
        update_data["summary"] = summary
    
    if tags is not None:
        update_data["tags"] = tags
        
    if source_url is not None:
        update_data["source_url"] = source_url
        
    if source_type is not None:
        update_data["source_type"] = source_type
    
    result = db.table("memories").update(update_data).eq("id", str(memory_id)).execute()
    
    return len(result.data) > 0 if result.data else False


async def delete_memory(
    db: Client,
    memory_id: UUID,
    user_id: UUID
) -> bool:
    """
    Delete a memory by ID.
    """
    result = db.table("memories").delete().eq("id", str(memory_id)).eq("user_id", str(user_id)).execute()
    
    return len(result.data) > 0 if result.data else False
